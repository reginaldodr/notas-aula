...
\item Determine uma equação vetorial que represente a curva obtida pela interseção dos dois parabolóides: $z=2x^2+\frac{y^2}{2}$ e $z=4-2x^2-\frac{3y^2}{2}$.
\end{enumerate}
\end{casab}

\newpage

\marcador{subsection}{Derivadas}

%\begin{tikzpicture}[remember picture, overlay]
%  \foreach \x in {0,0.5,...,21} { % horizontal (ajuste o passo para mudar densidade)
%    \foreach \y in {0,0.5,...,27} { % vertical (A4: ~29.7cm)
%      \fill[gray] (current page.south west)++(\x cm,\y cm) circle (0.5pt);
%    }
%  }
%\end{tikzpicture}

\begin{definb}{}{}
A {\color{blue}derivada} de uma função vetorial $\vec{r}$ é definida por:
\[\frac{d\vec{r}}{dt}=\vec{r}\,'(t)=\lim\limits_{h\to 0}\frac{\vec{r}(t+h)-r(t)}{h},\]
se esse limite existir.
\end{definb}
\vfill

\begin{caixaazul}{Interpretação Geométrica}
O vetor $\vec{r}\,'(t)$ é um {\color{blue}vetor tangente} à curva parametrizada por $\vec{r}$ no ponto $P=\vec{r}(t)$.
\end{caixaazul}
...
\newpage
\marcador{subsection}{Regras de Derivação}
...
\marcador{subsection}{Comprimento de arco}
	...
	\[{\color{blue}L}=\int_a^b\|{\color{red}\vec{r}\,'(t)}\|\,dt\]
\begin{exeb}{}{}
